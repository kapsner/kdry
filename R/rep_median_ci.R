...
rep_median_ci <- function(
  ...
  stopifnot(
    ...
    "`conf_int` must be in range between 0 and 100" = conf_int > 0 &&
      conf_int < 100,
    "`weighted` must be a boolean" = is.logical(weighted),
    "`weights` must be a numeric vector" = ifelse(
      weighted,
      is.vector(weights),
      TRUE
    )
  ...
  if (gsub(pattern = "\\s", replacement = "", x = collapse) %in% c("to", "-")) {
    ...
    gsub(pattern = "\\s", replacement = "", x = collapse) %in%
      c(";", ",")
  ) {
    collapse <- gsub(pattern = "\\s", replacement = "", x = collapse)
    collapse <- paste0(collapse, " ")
